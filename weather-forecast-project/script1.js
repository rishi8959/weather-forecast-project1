...
const API_KEY = "bcbab4ab01461cb66370405addc0550c"; // API key for OpenWeatherMap API
...
// Recent searches changes
// Load recent cities from localStorage and update the dropdown
function getWeatherDetFrmRecent(city){
    if (city === "") return;
...
function loadRecentCities() {
    ...
    recentCitiesDropdown.innerHTML = ''; // Clear previous entries
    ...
    if (recentCities.length > 0) {
        ...
        recentCities.forEach(city => {
            ...
            cityButton.addEventListener('click', () => {
                getWeatherDetFrmRecent(city);
                toggleDropdown(); // Hide dropdown after selection
            });
            recentCitiesDropdown.appendChild(cityButton);
        });
    } else {
...
document.addEventListener('click', (event) => {
    if (!event.target.closest('.dropdown-container')) {
        if (recentCitiesDropdown.classList.contains('show')) {
            toggleDropdown();
        }
    }
});
